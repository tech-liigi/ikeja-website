    ...
    return (
        <section className={`flex flex-col justify-start items-center w-full max-h-screen ${open ? 'overflow-y-auto' : 'overflow-y-hidden'}`}>
            <div className="w-full flex justify-between bg-black px-10 relative">
                <Link href={"/"}>
                <Image  loading={"lazy"} className='scale-105' src={"/Ikeja_FC_Logo.png"} alt={'Ikeja_LoGo'} title={"Ikeja"} width={80} height={80} quality={100}/>
                </Link>
...
const Header = (props:
    ...
    const [isMobile, setIsMobile] = useState<boolean>(typeof window !== 'undefined' && window.innerWidth < 950);
    ...
    return (
        ...
        <header>
            <div className={"sm:block md:block lg:hidden xl:hidden"}>
                <div className='w-full fixed top-0 z-10 drop-shadow-md'><MobileMainHeader/><SubHeader els={props.els}/></div>
            </div>
            <div className={"sm:hidden md:hidden lg:block xl:block"}>
            <><div className="w-full top-0 z-10 absolute drop-shadow-md max-h-[180px]" id="header"><MainHeader paddingResize={active}/><SubHeader els={props.els}/></div></>
            </div>
        </header>
    )
}
